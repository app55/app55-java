...
/**
 * The <code>Introspector</code> is a utility for developers to figure out which properties, events, and methods a JavaBean supports.
 * <p>
 * The <code>Introspector</code> class walks over the class/superclass chain of the target bean class. At each level it checks if there is a matching
 * <code>BeanInfo</code> class which provides explicit information about the bean, and if so uses that explicit information. Otherwise it uses the low level
 ...
 * </p>
 * <ol>
 * <li>The <code>Introspector</code> appends "BeanInfo" to the qualified name of the bean class, try to use the new class as the "BeanInfo" class. If the
 * "BeanInfo" class exsits and returns non-null value when queried for explicit information, use the explicit information</li>
 * <li>If the first step fails, the <code>Introspector</code> will extract a simple class name of the bean class by removing the package name from the qualified
 ...
 * "BeanInfo" search path is <code>sun.beans.infos</code>). If it finds a "BeanInfo" class and the "BeanInfo" class returns non-null value when queried for
 * explicit information, use the explicit information</li>
 * </ol>
 * 
 */
...
public class Introspector extends java.lang.Object
{
	...
	// Public fields
	/**
	 * Constant values to indicate that the <code>Introspector</code> will ignore all <code>BeanInfo</code> class.
	 */
	public static final int							IGNORE_ALL_BEANINFO			= 3;

	/**
	 * Constant values to indicate that the <code>Introspector</code> will ignore the <code>BeanInfo</code> class of the current bean class.
	 */
	public static final int							IGNORE_IMMEDIATE_BEANINFO	= 2;

	/**
	 * Constant values to indicate that the <code>Introspector</code> will use all <code>BeanInfo</code> class which have been found. This is the default one.
	 */
	 ...
	 * <li>UNLESS the second character is also Upper Case, when the String is returned unchanged <eul>
	 * 
	 * @param name
	...
	/**
	 * Gets the <code>BeanInfo</code> object which contains the information of the properties, events and methods of the specified bean class. It will not
