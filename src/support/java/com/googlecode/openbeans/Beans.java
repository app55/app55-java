...
public class Beans
{
	...
	private static Object internalInstantiate(ClassLoader cls, String beanName, BeanContext context, Object initializer) throws IOException,
	 ...
	 * </p>
	 * <p>
	 * Firstly, The class <code>Beans</code> regards the bean name as a serialized object name. The class <code>Beans</code> convert bean name into pathname,
	 * append a suffix ".ser" to the pathname. then try to load the resource using the specified class loader. If <code>Beans</code> fails to load the resource,
	 * <code>Beans</code> will regard the <code>name</code> as a class name and construct a new instance of the bean class.
	 * </p>
	 * <p>
	 * For example, if the name is specified as "x.y.z", The class <code>Beans</code> will try to load the serialized object from the resource "x/y/z.ser"; If
	 * <code>Beans</code> fails to load the resource "x/y/z.ser", it will create a new instance of "x.y.z".
	 * </p>
	 * <p>
	 * If the bean is an instance of java.applet.Applet, <code>Beans</code> will do some special initialization for this applet bean. First, <code>Beans</code>
	 * will set the default AppletStub and AppletContext for the applet bean (If the specified <code>AppletInitializer</code> is not null, <code>Beans</code>
	 * will call the <code>AppletInitializer.initialize</code> to set the default AppletStub and AppletContext for the applet bean). Second, <code>Beans</code>
	 * will call the <code>init</code> method of the applet. (If the applet bean is loaded as a serialized object, the <code>init</code> method will not be
	 * called.)
	 * </p>
	 * 
	 * @param cls
	 ...
	 *            should be <code>true</code> to signify that a GUI is available, <code>false</code> otherwise.
	...
	private static void appletLoaded(Applet applet, ClassLoader loader, String name, BeanContext context, AppletInitializer initializer, boolean deserialized)
			throws ClassNotFoundException
	{
